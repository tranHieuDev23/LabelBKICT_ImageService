...
export class BookmarkManagementOperatorImpl
    implements BookmarkManagementOperator
{
    constructor(
    ...
    public async createImageBookmark(
        ...
        if (image === null) {
            ...
            throw new ErrorWithStatus(
                `no image with image_id ${imageId} found`,
                status.NOT_FOUND
            );
        }
        description = this.sanitizeDescription(description);
        return await this.userBookmarksImageDM.withTransaction(
            async (userBookmarksImageDM) => {
                const bookmark =
                    await userBookmarksImageDM.getUserBookmarksImageWithXLock(
                        userId,
                        imageId
                    );
                if (bookmark !== null) {
                ...
                await userBookmarksImageDM.createUserBookmarksImage(
                    userId,
                    imageId,
                    description
                );
    ...
    public async getImageBookmark(
        ...
        const bookmark =
            await this.userBookmarksImageDM.getUserBookmarksImageWithXLock(
                userId,
                imageId
            );
        if (bookmark === null) {
            ...
            throw new ErrorWithStatus(
                `user ${userId} has not bookmarked image ${imageId}`,
                status.NOT_FOUND
            );
        }
        return bookmark;
    ...
    public async deleteImageBookmark(
        ...
        return await this.userBookmarksImageDM.deleteUserBookmarksImage(
            userId,
            imageId
        );
    }
...
export const BOOKMARK_MANAGEMENT_OPERATOR_TOKEN =
    token<BookmarkManagementOperator>("BookmarkManagementOperator");
